...
3. Add both entry points to the root `nomad.yaml`:
   ...
   plugins:
     entry_points:
       include:
         ...
         - "nomad_eosc_galaxy_actions.schema_packages:schema_package_entry_point"
       options:
         nomad_eosc_galaxy_actions.actions.find_peaks:find_peaks_action:
   ...
   `nomad-distro-dev`'s `nomad.yaml` uses an explicit `include` allowlist — nothing runs until it's listed here, even if the package is installed.
...
- Ensure that both both entry points are activated in your NOMAD configuration (e.g., your `nomad.yaml`).
